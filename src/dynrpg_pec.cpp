...
static bool GetColor(const dyn_arg_list& args) {
	...
	if (out_red > 0) {
		Game_Variables.Set(out_red, (int)(255.0 / tone.red * 100));
	}
	if (out_green > 0) {
		Game_Variables.Set(out_green, (int)(255.0 / tone.green * 100));
	}
	if (out_blue > 0) {
		Game_Variables.Set(out_blue, (int)(255.0 / tone.blue * 100));
	}
	if (out_saturation > 0) {
		Game_Variables.Set(out_saturation, (int)(255.0 / tone.gray * 100));
	}
...
static bool GetPosition(const dyn_arg_list& args) {
	...
	if (out_x > 0) {
		Game_Variables.Set(out_x, sprite->GetX());
	}
	if (out_y > 0) {
		Game_Variables.Set(out_y, sprite->GetY());
	}
...
static bool GetTransparency(const dyn_arg_list& args) {
	...
	if (out_transparency > 0) {
		...
		Game_Variables.Set(out_transparency, o);
	}
...
static bool GetRotation(const dyn_arg_list& args) {
	...
	if (out_angle > 0) {
		...
		Game_Variables.Set(out_angle, a);
	}
...
static bool GetEffectStrength(const dyn_arg_list& args) {
	...
	if (out_strength > 0) {
		Game_Variables.Set(out_strength, sprite->GetWaverDepth());
	}
...
static bool IsFlippedHorizontal(const dyn_arg_list& args) {
	...
	if (out_flipped > 0) {
		Game_Variables.Set(out_flipped, sprite->GetFlipX());
	}
