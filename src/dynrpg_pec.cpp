...
static bool SetColor(const dyn_arg_list& args) {
	...
	const Sprite* sprite = Main_Data::game_pictures->GetPicture(picture_id).GetSprite();
...
static bool GetColor(const dyn_arg_list& args) {
	...
	if (out_red > 0) {
		Main_Data::game_variables->Set(out_red, (int)(255.0 / tone.red * 100));
	}
	if (out_green > 0) {
		Main_Data::game_variables->Set(out_green, (int)(255.0 / tone.green * 100));
	}
	if (out_blue > 0) {
		Main_Data::game_variables->Set(out_blue, (int)(255.0 / tone.blue * 100));
	}
	if (out_saturation > 0) {
		Main_Data::game_variables->Set(out_saturation, (int)(255.0 / tone.gray * 100));
	}
...
static bool GetPosition(const dyn_arg_list& args) {
	...
	if (out_x > 0) {
		Main_Data::game_variables->Set(out_x, sprite->GetX());
	}
	if (out_y > 0) {
		Main_Data::game_variables->Set(out_y, sprite->GetY());
	}
...
static bool GetTransparency(const dyn_arg_list& args) {
	...
	if (out_transparency > 0) {
		...
		Main_Data::game_variables->Set(out_transparency, o);
	}
...
static bool GetZoom(const dyn_arg_list& args) {
	...
	if (out_zoom > 0) {
		...
		Main_Data::game_variables->Set(out_zoom, o);
	}
...
static bool GetRotation(const dyn_arg_list& args) {
	...
	if (out_angle > 0) {
		...
		Main_Data::game_variables->Set(out_angle, a);
	}
...
static bool GetEffectStrength(const dyn_arg_list& args) {
	...
	if (out_strength > 0) {
		Main_Data::game_variables->Set(out_strength, sprite->GetWaverDepth());
	}
...
static bool IsFlippedHorizontal(const dyn_arg_list& args) {
	...
	if (out_flipped > 0) {
		//Main_Data::game_variables->Set(out_flipped, sprite->GetFlipX());
	}
...
static bool DestroySpritesheet(const dyn_arg_list& args) {
	...
	//FIXME sprite->SetSrcRect(sprite->GetBitmap()->GetRect());
